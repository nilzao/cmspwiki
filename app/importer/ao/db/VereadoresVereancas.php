<?php
class app_importer_ao_db_VereadoresVereancas{
	private $dataBase;
	...
	public function upsert(app_importer_bean_db_VereadoresVereancas $objmodel){
		if ($objmodel->id == 0){
			$array = array($objmodel->id_vereador,
						   $objmodel->id_vereador_anterior,
						   $objmodel->getData_ini(),
						   $objmodel->getData_fim(),
						   $objmodel->situacao,
						   $objmodel->partido,
						   $objmodel->partido_obs,
						   $objmodel->obs);
			$query = "INSERT INTO vereadores_vereancas
			(id_vereador,id_vereador_anterior,data_ini,data_fim,situacao,partido,partido_obs,obs)
			VALUES (?,?,?,?,?,?,?,?)";
	...
	public function truncate(){
		$query = "TRUNCATE vereadores_vereancas";
